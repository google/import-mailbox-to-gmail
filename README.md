...
* To run under [Docker](https://www.docker.com/): First build the image, `docker build -t google/import-mailbox-to-gmail .`; then run the import command, `docker run --rm -it -v "/local/path/to/auth.json:/auth.json" -v "/local/path/to/mbox:/mbox" google/import-mailbox-to-gmail --json "/auth.json" --dir "/mbox"`
